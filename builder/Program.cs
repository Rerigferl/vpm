...
unsafe
{
    ConsoleApp.Run(args, &Command.Root);
}

class Command
{
    /// <param name="listPath">Source file path</param>
    /// <param name="repositoryToken">-r, ${{ secrets.GITHUB_TOKEN }}</param>
    public static async void Root([Argument] string listPath, string? repositoryToken)
    {
    ...
    private async static Task<string> GetSHA256(HttpClient client, Asset zip)
    {
    ...
    private static readonly ProductInfoHeaderValue UserAgent = new("numeira.vpm-repository-builder", null);
